...
import { getReportBuilderMockContext, segmentFrameWithSam3 } from '@/api/reportBuilder'
...
const videoAspectRatio = ref(4 / 3)
...
const applyContext = (nextContext: ReportContextData) => {
  ...
  videoAspectRatio.value = nextContext.videoFrameData.width && nextContext.videoFrameData.height
    ? nextContext.videoFrameData.width / nextContext.videoFrameData.height
    : 4 / 3
}
...
const handleCaptureFrame = async (payload: CaptureFramePayload) => {
  ...
  try {
    ...
    const frameWidth = context.value?.videoFrameData.width ?? 1280
    const frameHeight = context.value?.videoFrameData.height ?? 720
...
<template>
  <main class="mx-auto flex h-[calc(100vh-88px)] w-full max-w-[1920px] flex-col overflow-hidden px-4 py-3 lg:px-6 lg:py-4">
    <ReportBuilderWorkspace
      ...
      @video-aspect-ratio-change="videoAspectRatio = $event"
